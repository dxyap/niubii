...
def calculate_monthly_returns(equity_curve: pd.Series) -> pd.DataFrame:
    ...
    """
    # Resample to month-end (using 'ME' for month-end frequency)
    ...
    # Add yearly total (using 'YE' for year-end frequency)
    yearly = equity_curve.resample("YE").last().pct_change() * 100
