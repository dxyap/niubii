...
def generate_html_report(result: BacktestResult) -> str:
    ...
    html.append("<html>")
    html.append("<head>")
    html.append(f"<title>Backtest Report - {result.strategy_name}</title>")
    html.append("""
    <style>
        body {
...
def create_equity_chart(result: BacktestResult, height: int = 400) -> Any | None:
    """
...
def create_drawdown_chart(result: BacktestResult, height: int = 250) -> Any | None:
    """
...
def create_returns_distribution(result: BacktestResult, height: int = 300) -> Any | None:
    """
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor=COLORS["background"],
        xaxis={"title": "Daily Return (%)", "gridcolor": COLORS["grid"]},
        yaxis={"title": "Frequency", "gridcolor": COLORS["grid"]},
    )

    return fig


def create_monthly_heatmap(result: BacktestResult, height: int = 350) -> Any | None:
    """
...
def create_trade_analysis_chart(result: BacktestResult, height: int = 300) -> Any | None:
    """
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor=COLORS["background"],
        xaxis={"title": "Trade #", "gridcolor": COLORS["grid"]},
        yaxis={"title": "P&L ($)", "gridcolor": COLORS["grid"], "tickformat": "$,.0f"},
        barmode="overlay",
    )
