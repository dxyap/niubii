...
class AlertEngine:
    ...
    def __init__(
        ...
        self._last_escalation_check: Optional[datetime] = None
    ...
    def resolve(self, alert_id: str, note: Optional[str] = None) -> bool:
        """Resolve an alert."""
        with self._lock:
            if alert_id in self._active_alerts:
                event = self._active_alerts[alert_id]
    ...
    def check_escalations(self):
        ...
        with self._lock:
            for alert_id, event in list(self._active_alerts.items()):
                if event.escalated:
                    continue
    ...
    def mute(self, duration_minutes: Optional[int] = None):
        ...
        logger.info(f"Alerts muted" + (f" for {duration_minutes} minutes" if duration_minutes else ""))
