...
class SimulatedBroker(Broker):
    ...
    def _check_limit_order(self, order: SimulatedOrder):
        ...
        if order.order_type == "LIMIT":
            if order.side == "BUY" and order.limit_price and price <= order.limit_price:
                can_fill = True
            elif order.side == "SELL" and order.limit_price and price >= order.limit_price:
                can_fill = True
    ...
    def _process_fill(self, order: SimulatedOrder, quantity: int, price: float):
        ...
        # Update cash
        notional = quantity * price * self.sim_config.contract_multiplier
        if order.side == "BUY":
    ...
    def get_positions(self, account: Optional[str] = None) -> Dict[str, Dict]:
        """Get all positions."""
        result = {}
        for symbol, pos in self._positions.items():
            if pos.quantity != 0:
